...
for tmp_j in range(Nat):
    na_nb_list = nanb.nanb(sym[tmp_j])
    a = na_nb_list[0]
    b = na_nb_list[1]
    val["Ntotal"] = val["Ntotal"] + a + b
    #print(tmp_j,a,b)
    val["Ntotale"] = val["Ntotale"] + atno.atno(sym[tmp_j])
    val["Ntotalecore"] = val["Ntotalecore"] + NFC.NFC(sym[tmp_j])
...
uniqat_d = uniqatoms.uniqatoms(sym)
    ...
    with open("geom_cation_IP.xyz", "r") as g_cat:  # need to look at this file
        with open("inp.xyz", "w") as inp_x:
            ...
            for tmp_k in range(Nat):
                na_nb_l = nanb.nanb(sym[tmp_k])
                na = na_nb_l[0]
                nb = na_nb_l[1]
                val["Ntotal"] = val["Ntotal"] + na + nb
                val["Ntotale"] = val["Ntotale"] + atno.atno(sym[tmp_k])
                #print(atno(sym[tmp_k]), val["Ntotale"] )
                val["Ntotalecore"] = val["Ntotalecore"] + NFC.NFC(sym[tmp_k])
...
if val["calc_AE"] == "true":
    ...
    for tmp_o in range(uniq_atom_res["N_ua"]):
        ...
        na_nb_l = nanb.nanb(sym[0])
        na = na_nb_l[0]
        nb = na_nb_l[1]
        val["Ntotal"] = na + nb

        val["Ntotale"] = atno.atno(ua[tmp_o])
        val["Ntotalecore"] = NFC.NFC(ua[tmp_o])
        val["Na"] = (multip + val["Ntotal"] - 1 ) / 2.0
        val["Nb"] = val["Na"] + 1 - multip
...
if val["calc_HF"] == "true":
    ...
    for tmp_u in range(uniq_atom_res["N_ua"]):
        HOF_0K.append(HOF_atoms.HOF_atoms(ua[tmp_u]))
        dH_298K_0K.append(dH_atoms.dH_atoms(ua[tmp_u]))
